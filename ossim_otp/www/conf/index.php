    ...
    'Login' => array(
        ...
        'conf' => array(
            ...
            'login_ldap_bindDN' => array(
                'type' => 'text',
                'help' => _('Account to search the user in LDAP<br />Example: cn=admin,dc=local,dc=domain,dc=net'),
                'desc' => _('Ldap bindDN'),
                'advanced' => 1 ,
                'section' => 'users'
            ),
            'login_ldap_valid_pass' => array(
                'type' => 'password',
                'help' => _('Password of Ldap bindDN'),
                'desc' => _('Ldap password for bindDN'),
                'advanced' => 1 ,
                'section' => 'users'
            ),
    ...
    'OTX' => array(
        ...
        'conf' => array(
            'open_threat_exchange' => array(
                ...
                'section' => 'otx',
                'onchange' => ($conf->get_conf('open_threat_exchange') != 'yes' && $conf->get_conf('open_threat_exchange_key') == '') ? "if (this.value=='yes') window.open('/ossim/updates/otxform.php'); else window.open('/ossim/updates/otxform.php?join=2')" : "enable_disable(this.value, 'otx')",
                'advanced' => 1
            ),
            'open_threat_exchange_key' => array(
            	...
            	'value' => "<span class='otx' ".(($conf->get_conf('open_threat_exchange') == 'yes') ? '' : "style='color:gray'").">".$conf->get_conf('open_threat_exchange_key')."</span>" ,
                'advanced' => 1
            ),
...
if (POST('update'))
{
	...
	for ($i = 0; $i < POST('nconfs'); $i++)
	{
		...
		if(in_array(POST("conf_$i"), $numeric_values) && intval(POST("value_$i")) < 0)
		{
            ...
            $warning_string     .= " "._("Invalid $variable, it has to be greater than zero.");
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="/ossim/style/av_common.css"/>
	...
	<script type="text/javascript" src="/ossim/js/jquery.placeholder.js"></script>
	<script type="text/javascript" src="/ossim/js/accordian.js"></script>
...
<body>
	...
	<table align='center' style='margin-top:17px;'>
		...
		<td class="noborder">
			<div id="basic-accordian" align="center">
				...
				{ 
					...
					<div id="test<?php
						if ($div > 0) echo $div ?>-header" class="accordion_headings 
						<?php if ($found == 1) echo " header_found" ?>">
						<?php echo $val["title"] ?>
					</div>
					...
					<div id="test<?php
						...
						<div class="accordion_child">
							<table class='conf_items' cellpadding='3' align="center" width="100%">
							<?php				            
							...
							foreach($val['conf'] as $conf => $type) 
							{
								if ($advanced || ($section == '' && !$advanced && $type['advanced'] == 0) || ($section != "" && preg_match("/$section/",$type['section'])))
								{
									...
									<tr <?php if (in_array($conf, $arr)) echo "bgcolor=#DFF2BF" ?>>
										...
										<td <?php if ($type['style'] != "") echo "style='".$type['style']."'" ?> <?php if ($type['classname'] != "") echo "class='".$type['classname']."'" ?>>
										...
										<td align="right">															
											...
											?>												
											<img src="../pixmaps/help.png" width="16" id='<?php echo $help_id?>' class='conf_help' border='0'/>
			...
			<td valign='top' class="noborder right">
				<?php 
                    submit(); 
                ?>
